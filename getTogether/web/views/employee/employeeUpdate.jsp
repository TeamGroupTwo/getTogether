...
<body>
	...
	<div id="container">
		<form action="<%=request.getContextPath()%>/update.emp" method="POST" id="updateForm" encType="multipart/form-data">
		  	...
		  	<div id="fileBox">
		  		...
		  		<input type="text" name="defaultProfile" id="rProfile" hidden="hidden"/>
		  	</div>
			...
			<div class="fu-btn fu-btn-font" id="updateBtn" onclick="updateEmployee();">확인</div>		
		</form>
	</div>
	...
	<script>
		...
		$(function(){
			...
			$('#rProfile').val(sessionStorage.getItem('loginProfile'));
			
			
			var email = sessionStorage.getItem('loginEmail');
			var emailAccount = email.substring(email.lastIndexOf('@')+1, email.length);
			
			var self = 0;
			
			$('#emailList').children().each(function(){
				if(emailAccount == $(this).val()){
					$(this).attr("selected","selected");
					$('#email').val(email.substring(0, email.lastIndexOf('@')));
					self = 1;
				}
			});
			...
			if(self == 0){
				...
				$("#selfEmail").val(email);
				$('#emailList').children().attr("selected", "selected");
			}
				
			$('#phone').val(sessionStorage.getItem('loginPhone'));
			
			var myProfile = sessionStorage.getItem('loginProfile');
			
			if(myProfile == "undefined")
	 			$("#my-profile").css("content", "url('/gt/resources/images/common/profile.png')");
	 		else{ 			
				$("#file-text").val(myProfile);
		...
		function onSubmit() {
			...
			if($("#file-text").val() != sessionStorage.getItem("loginProfile"))
				sessionStorage.setItem('loginProfile', $("#file-text").val());
			
			if($("#rEmail").val() != "")
				sessionStorage.setItem('loginEmail', $("#rEmail").val());				
			else
				$("#rEmail").val(sessionStorage.getItem("loginEmail"));
